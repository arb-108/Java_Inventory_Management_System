...
/**
 * Servlet implementation class checkReturnCart
 */
@WebServlet("/checkReturnCart")
public class checkReturnCart extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public checkReturnCart() {
        super();
		 ...
		 if (session == null || session.getAttribute("usertype") == null) {
			...
			ArrayList<Product> items=(ArrayList<Product>) session.getAttribute("Returnitems");
			 PrintWriter out=response.getWriter();
		        ...
		        out.println("<body>");
		        out.println("<h2>Return Cart</h2>");
		        out.println("<button><a href='UserServlet' style='text-decoration: none; color: inherit;'>Go to Dashboard</a></button>");
		        out.println("<button><a href='returnsServlet' style='text-decoration: none; color: inherit;'>Returns</a></button>");
				...
				if(items==null || items.isEmpty()) {
					out.println("<br>");
					out.println("No Product in Return Cart");
				}else {
					double totalPrice=0.0;
					out.println("<h3>Return Products in Cart:</h3>");
				    out.println("<table border='1'>");
				    out.println("<tr><th>Order ID</th><th>ID</th><th>Category</th><th>Name</th><th>Quantity</th><th>Price</th><th>Actions</th></tr>");
				    for(Product product :items ) {
				        out.println("<tr>");
				        out.println("<td>" + product.getOid()+ "</td>");
					    ...
					    out.println("<form action='RemoveFromReturnCart' method='get' style='display: inline;'>");
				...
				out.println("<form action='ReturnsSaveDatabase' method='post' style='display: inline;'> ");
